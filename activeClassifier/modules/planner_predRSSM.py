...
from tools.tf_tools import repeat_axis, entropy, differential_entropy_normal
...
class ActInfPlanner(Planner):
    ...
    def planning_step(self, current_state, z_samples, time, is_training):
        ...
        with tf.name_scope('Planning_loop'):  # loop over policies, parallised into [B * self.n_policies, ...]
            ...
            next_actions, next_actions_mean = self.m['policyNet'].next_actions(time=time, inputs=[current_state['c'], current_state['s']], is_training=is_training, n_policies=self.n_policies, policy_dep_input=self.policy_dep_input)
            ...
            with tf.name_scope('Hypotheses_loop'):  # for every action: loop over hypotheses
                ...
                # next_actions = tf.Print(next_actions, [tf.reduce_mean(exp_obs), tf.reduce_mean(H), tf.reduce_mean(exp_H)])
            ...
            # preference_error = tf.Print(preference_error, [tf.reduce_mean(exp_H), tf.reduce_mean(H_exp_exp_obs), tf.reduce_mean(preference_error)])
